...
- Removed the "Focus" settings section. Its behaviors are now sensible always-on defaults: end-of-day wins banner, routine streaks + celebration, and the ~free-time threshold in the agenda (10 min buffer). The "Layout density" setting was removed outright — it only affected the removed timeline layout, which is why changing it appeared to do nothing.
