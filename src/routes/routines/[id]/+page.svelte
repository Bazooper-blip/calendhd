<script lang="ts">
	...
	// Populate form from existing routine (runs once)
	$effect(() => {
		if (!initialized && routine) {
			name = routine.name;
			icon = routine.icon ?? '';
	...
	function removeStep(index: number) {
		if (steps.length > 1) steps = steps.filter((_, i) => i !== index);
	}
	...
	async function handleSave() {
		if (!canSave || saving || !routineId) return;
		saving = true;
		try {
			await routinesStore.update(routineId, {
				name,
...
<div class="h-full overflow-y-auto">
	<div class="max-w-2xl mx-auto px-4 py-6 space-y-6">
		...
		{#if !routine && routinesStore.loading}
		...
		{:else if !routine}
			<div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-100 dark:border-neutral-700 p-8 text-center">
				<p class="text-neutral-500 dark:text-neutral-400">{$t('errors.notFound')}</p>
				<Button class="mt-4" onclick={() => goto('/routines')}>{$t('common.back')}</Button>
			</div>
		{:else}
			...
			<div class="bg-white dark:bg-neutral-800 rounded-xl shadow-sm border border-neutral-100 dark:border-neutral-700 p-4 space-y-4">
				...
				<div>
					<p class="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">{$t('event.icon')}</p>
					<IconPicker value={icon} onSelect={(v) => (icon = v)} />
				</div>

				<div>
					<p class="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">{$t('event.color')}</p>
