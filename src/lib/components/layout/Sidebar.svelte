<script lang="ts">
	...
	async function resumeEvent(id: string) {
		try {
			await calendar.updateEvent(id, { is_paused: false });
		...
		<nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
			...
			{#if calendar.pausedEvents.length > 0}
				<div class="pt-4 mt-4 border-t border-neutral-100 dark:border-neutral-700">
					...
					<div class="mt-2 space-y-1">
						{#each calendar.pausedEvents as event (event.id)}
							<div class="flex items-center gap-2 px-3 py-2">
								...
								<a
									href={`/event/${event.id}`}
									onclick={onClose}
									class="flex-1 min-w-0 truncate text-sm text-neutral-600 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200"
								>
									{event.title}
								</a>
								<button
