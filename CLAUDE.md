...
5. **Every `onRecord*` handler MUST call `e.next()`.** PB chains handlers; a handler that skips `e.next()` silently stops the chain — no error, no log — so only the handlers loaded before it run. Worse, hook **file load order is filesystem-dependent** (empirically NOT reliably alphabetical: rewriting a file changed which handler won), so with `e.next()` missing everywhere exactly one arbitrary handler per (hook, collection) survives. This bit production: from 1.9.3, `025_new_event_notification` suppressed `010_reminder_scheduler` on events-create, so newly created events never got reminders scheduled (fixed in 1.10.0; verified on PB 0.39.11 and 0.40.1). Enforced twice: `node pocketbase/tests/hookChain.test.cjs` (static) and `node pocketbase/tests/integration.live.cjs` (boots the real binary and asserts BOTH events-create side effects, so suppression fails CI whichever handler survives).
