...
5. **Every `onRecord*` handler MUST call `e.next()`.** PB chains handlers; a handler that skips `e.next()` silently suppresses every other handler bound to the same (hook, collection) — no error, no log. This bit production: from 1.9.3, `025_new_event_notification` ran ahead of `010_reminder_scheduler` on events-create and swallowed it, so newly created events never got reminders scheduled (fixed in 1.10.0; verified empirically on PB 0.39.11 and 0.40.1). `node pocketbase/tests/hookChain.test.cjs` statically enforces it — keep it passing when adding hooks.
